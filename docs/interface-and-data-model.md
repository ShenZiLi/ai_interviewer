...
> 更新时间：2026-09-20 ｜ 状态：M1 接口基线已落地，M2 身份与持久化细化中 ｜ 上游：[需求与决策](requirements.md)、[系统架构与技术选型](system-architecture.md)
>
> 本文记录已确认的数据模型基线。标注「待确认」的条目属于 M2 产品决策；当前可运行路由的 OpenAPI 见 [`openapi.yaml`](openapi.yaml)。
...
> P01—P10 各自的输出骨架、字段枚举与校验规则已定义，见 [输出 Schema 与校验规则](output-schemas.md)。
