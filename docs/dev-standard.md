...
- **第三方处理授权**：简历首次发送 GLM 前必须有单独勾选；保存授权时间与说明版本；模型请求正文不得进入应用日志。
...
| AC10 | 账号安全 | 用户名唯一；密码仅存 Argon2id 哈希；错误响应与日志不泄露密码、哈希或 JWT |
| AC11 | 简历授权 | 未勾选时不得调用 GLM；勾选后记录授权时间与说明版本；日志不含简历正文或模型请求正文 |

### 5.2 范围外（M1 不做）

微信小程序端、录音长期保留、提示词管理后台（含草稿、测试、发布、回滚）、端到端实时通道、简历多版本/OCR、微信扫码/短信等扩展登录渠道。
...
3. M1 全绿后进入 M2（微信扫码/短信等扩展登录渠道、小程序端、跨端同步增强）。
