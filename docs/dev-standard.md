...
> 验证命令：`pnpm run smoke`（typecheck + 全仓 test + web 构建；当前 118 测试全绿：contracts 15 / web 16 / server 87）。
...
login/DB/持久化重启与否、录音长期保留、管理员真实 LLM 测试、端到端实时通道、简历多版本/OCR 属范围外（M2）。已超出 MVP 额外交付：设置页运行期切换文本供应商 + 连通性测试（[api-spec §5.1](api-spec.md)）、进行中面试一键继续、整场八维归一、回答转写回顾按环节分组/追问挂主问题、简历结构化确认与方向多选、compose 校验失败降级 502、会话状态复位与可读错误提示、整场方向覆盖按主题去重、评分随作答变化；**复盘与进步反馈**：报告「vs 上一场」逐维趋势、工作台「成绩走势」与按目标岗位筛选、mock 评分按作答分化使趋势默认可见、报告「本场最佳/最需改进」作答高亮（P10 用真实轮 id）；**隐私与生命周期**：录音保留策略（默认会话结束即删/keepAudio 保留/删除即弃/孤立音频清扫）、保留录音在复盘「回答转写回顾」回听（`GET /files/audio/:ref`）；**面试体验**：主问题出题元信息（主题/难度/考察维度）与阶段进度、mock 出题按阶段轮转、陪练反馈阅读与重答耗时、单轮反馈「误区澄清」（知识/转写/假设）、准备页题目预览、面试风格选项（设置页存默认 + 准备页本场可调，仅影响反馈文案口径不改评分）；**本地开发**：后端启用 CORS（反射请求源）支持前端 5173 直连 3000。
