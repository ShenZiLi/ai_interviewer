...
打开 http://localhost:5173 体验：工作台 → 我的简历（导入/粘贴 → 解析）→ 准备面试（可选填目标岗位 JD 与补充诉求 → 选方向/风格/模式/时长 → 生成流程并预览题目 → 开考）→ 面试室（录音或文本作答 → 评分/误区澄清/追问/重答/同环节再问一题 →）→ 复盘报告（八维与逐题自洽的整场归一、回答转写回顾（保留录音时可回听）、报告「vs 上一场」逐维趋势、本场最佳/最需改进作答高亮、工作台「成绩走势」「平均八维」与按目标岗位筛选）；侧栏「提示词管理」可维护 P01—P10 模板（草稿/示例测试/发布/回滚/删除，开场快照锁定版本）。工作台可对「进行中」的面试一键继续，中途离开不回丢失进度。
